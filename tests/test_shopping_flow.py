...
class TestShoppingFlow:
    ...
    def test_01_home_index(self, api_client: APIClient, shopping_data: dict):
        ...
        response = api_client.get(Endpoints.HOME_INDEX)
    ...
    def test_03_create_address(self, authenticated_client: APIClient, shopping_data: dict):
        ...
        body = response.json()
        assert body["errno"] == case["expected"]["errno"], \
            f"创建地址失败: {body.get('errmsg')}"

        # data 直接是地址 ID（整数）
        TestShoppingFlow.address_id = body["data"]
    ...
    def test_04_cart_add(self, authenticated_client: APIClient, shopping_data: dict):
        ...
        response = authenticated_client.post(Endpoints.CART_ADD, json={
        ...
        body = response.json()
        assert body["errno"] == case["expected"]["errno"], \
            f"加购失败: {body.get('errmsg')}"

        cart_count = body["data"]
        logger.info(f"[4/7] 加入购物车 → 商品数={cart_count}")
    ...
    def test_05_cart_checkout(self, authenticated_client: APIClient, shopping_data: dict):
        ...
        body = response.json()
        assert body["errno"] == case["expected"]["errno"], \
            f"结算失败: {body.get('errmsg')}"
        data = body["data"]
        for field in case["expected"]["fields"]:
            assert field in data, f"缺少字段: {field}"

        TestShoppingFlow.cart_id = data.get("cartId", 0)
        TestShoppingFlow.coupon_id = data.get("couponId", -1)

        logger.info(
            f"[5/7] 结算 → 总价={data['goodsTotalPrice']}, "
            f"运费={data['freightPrice']}, 实付={data['actualPrice']}"
        )
    ...
    def test_06_order_submit(self, authenticated_client: APIClient, shopping_data: dict):
        ...
        response = authenticated_client.post(Endpoints.ORDER_SUBMIT, json={
        ...
        body = response.json()
        assert body["errno"] == case["expected"]["errno"], \
            f"下单失败: {body.get('errmsg')}"

        TestShoppingFlow.order_id = body["data"]["orderId"]
    ...
    def test_07_order_list(self, authenticated_client: APIClient, shopping_data: dict):
        ...
        response = authenticated_client.get(Endpoints.ORDER_LIST)

        APIAssertions.assert_status_code(response, 200)
        body = response.json()
        assert body["errno"] == case["expected"]["errno"]

        logger.info(f"[7/7] 订单列表 → 共 {body['data']['total']} 个订单")
